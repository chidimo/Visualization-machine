...
////////////////////////////
  ...
  Template.song_viz_controls.events({
    ...
    "change .js-select-single-feature":function(event){
      ...
      console.log(Session.get("feature"))
    }, 
    // event handler for when the user clicks on the 
    // blobs button
     "click .js-show-blobs":function(event){
      event.preventDefault();
      Session.set("visualization_description", "Blob visualization for " + Session.get("feature")["name"])
      initBlobVis();
    }, 
    // event handler for when the user clicks on the 
    // timeline button
     "click .js-show-timeline":function(event){
      event.preventDefault();
      Session.set("visualization_description", "Timeline visualization for " + Session.get("feature")["name"])
      initDateVis();
    }, 
  });

  Template.song_data_visualization.helpers({
    "description" : function() {
      return Session.get("visualization_description")
    }
  })
}
  ...
  // a node object that the visualiser can understand
    songs.forEach(function(song){
     ...
     if (song.metadata.tags.title != undefined){// we have a title
      ...
      var value = song[feature["type"]][feature["name"]];
